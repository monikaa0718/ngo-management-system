  ...
  const steps = [
    {
      title: "Personal Info",
      fields: [
        { name: "name", label: "Full Name", type: "text" as const, placeholder: "Enter volunteer name", required: true },
        { name: "email", label: "Email", type: "email" as const, placeholder: "volunteer@example.com" },
      ],
    },
    {
      title: "Contact & Skills",
      fields: [
        { name: "phone", label: "Phone", type: "tel" as const, placeholder: "+91 XXXXX XXXXX" },
        { name: "skills", label: "Skills", type: "textarea" as const, placeholder: "e.g., Teaching, First Aid, Driving" },
      ],
    },
  ];
    ...
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}>
        <h1 className="text-3xl font-display font-bold text-foreground flex items-center gap-3">
          <Users className="w-8 h-8 text-info" /> Volunteers
        </h1>
        <p className="text-muted-foreground mt-1">Register and manage volunteers</p>
      </motion.div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="glass-card rounded-xl p-6">
          <h3 className="font-display font-semibold text-foreground mb-4">Register Volunteer</h3>
          <VerticalStepForm steps={steps} onSubmit={(data) => addMutation.mutateAsync(data)} submitLabel="Register" />
        </motion.div>

        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="lg:col-span-2 glass-card rounded-xl p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-display font-semibold text-foreground">All Volunteers ({filtered.length})</h3>
          ...
          {isLoading ? (
            ...
            <p className="text-center text-muted-foreground py-12">No volunteers yet.</p>
          ) : (
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-border">
                    ...
                    <th className="text-left py-3 px-4 text-xs font-medium text-muted-foreground uppercase">Skills</th>
                    ...
                    <tr key={v.id} className="border-b border-border/50 hover:bg-muted/30 transition-colors">
                      {editingId === v.id ? (
                        <>
                          ...
                          <td className="py-2 px-4"><Input value={editData.skills || ""} onChange={(e) => setEditData({ ...editData, skills: e.target.value })} className="text-sm h-8" /></td>
                          <td className="py-2 px-4 text-right space-x-1">
                            <Button size="sm" variant="outline" onClick={() => setEditingId(null)}>Cancel</Button>
                            <Button size="sm" className="gradient-primary text-primary-foreground" onClick={() => updateMutation.mutate({ id: v.id, name: editData.name, email: editData.email, phone: editData.phone, skills: editData.skills })}>Save</Button>
                          </td>
                        </>
                      ) : (
                        <>
                          <td className="py-3 px-4 text-sm font-medium text-foreground">{v.name}</td>
                          <td className="py-3 px-4 text-sm text-muted-foreground">{v.email || "—"}</td>
                          <td className="py-3 px-4 text-sm text-muted-foreground">{v.phone || "—"}</td>
                          <td className="py-3 px-4 text-sm text-muted-foreground">{v.skills || "—"}</td>
                          <td className="py-3 px-4 text-right space-x-1">
                            <Button size="sm" variant="ghost" onClick={() => { setEditingId(v.id); setEditData(v); }}>
                              <Edit2 className="w-4 h-4" />
                            </Button>
                            {isAdmin && (
                              <Button size="sm" variant="ghost" className="text-destructive hover:text-destructive" onClick={() => deleteMutation.mutate(v.id)}>
